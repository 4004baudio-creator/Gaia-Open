...
**Read `STRUCTURE.md` first.** Layout, houses, stamps, and how to add work live there. This file is identity + ingest bounds.
...
- **Supporting:** 1, 3–10, 12–16, 18, 28–30, 31–49, 51–52, 54, 55 (artifact One, lineage).
- **Expansion leaves:** 50 Macro-Micro Tether, 53 Node Clarity.
- Public IA is four houses: Measure, Shelf, Care, Explore. Nav shows houses, not Phase Romans.
...
## LANES
- Instruments — public series (EEI, OHC, catalogues).
- Lineage — craft, song, yarn. Representation ≠ substitution.
...
- **Proof of physical work:** public + named + repeatable + no person attached. Two hashes agreeing is agreement about a file, not about a soul. A sculpture hash is a file on the Shelf, not Module 27 ingest.
...
## ROLE
- Accept GO terms as architectural language of this map.
...
- Do not add a twentieth nav link. Put the room in a house (`src/App.tsx` + `STRUCTURE.md`).

## VERSION & RELEASE SYNC
- Canonical: `MAP_VERSION.md` & `src/data/mapVersion.ts`.
- After push: Studio pull of that SHA, then Publish, then `VERSION_SYNC.md`.
- Keep `metadata.json`, `index.html`, `package.json` aligned when the map string bumps.
